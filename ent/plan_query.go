...
func (pq *PlanQuery) QueryUserpaymentplans() *UserPaymentPlanQuery {
	query := (&UserPaymentPlanClient{config: pq.config}).Query()
...
func (pq *PlanQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (pq *PlanQuery) Clone() *PlanQuery {
	...
	return &PlanQuery{
		...
		withUserpaymentplans: pq.withUserpaymentplans.Clone(),
...
func (pq *PlanQuery) WithUserpaymentplans(opts ...func(*UserPaymentPlanQuery)) *PlanQuery {
	query := (&UserPaymentPlanClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withUserpaymentplans = query
	return pq
}
...
func (pq *PlanQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Plan, error) {
	...
	if query := pq.withUserpaymentplans; query != nil {
		if err := pq.loadUserpaymentplans(ctx, query, nodes,
			func(n *Plan) { n.Edges.Userpaymentplans = []*UserPaymentPlan{} },
			func(n *Plan, e *UserPaymentPlan) { n.Edges.Userpaymentplans = append(n.Edges.Userpaymentplans, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PlanQuery) loadUserpaymentplans(ctx context.Context, query *UserPaymentPlanQuery, nodes []*Plan, init func(*Plan), assign func(*Plan, *UserPaymentPlan)) error {
	...
	query.Where(predicate.UserPaymentPlan(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(plan.UserpaymentplansColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.plan_userpaymentplans
		if fk == nil {
			return fmt.Errorf(`foreign-key "plan_userpaymentplans" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "plan_userpaymentplans" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

...
// QueryMainiots chains the current query on the "mainiots" edge.
func (uq *UserQuery) QueryMainiots() *MainIotQuery {
	query := (&MainIotClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithMainiots tells the query-builder to eager-load the nodes that are connected to
// the "mainiots" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithMainiots(opts ...func(*MainIotQuery)) *UserQuery {
	query := (&MainIotClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withMainiots; query != nil {
...
func (uq *UserQuery) loadRoles(ctx context.Context, query *RoleQuery, nodes []*User, init func(*User), assign func(*User, *Role)) error {
	...
	nids := make(map[int16]map[*User]struct{})
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				inValue := int16(values[1].(*sql.NullInt64).Int64)
				if nids[inValue] == nil {
...
func (uq *UserQuery) loadMainiots(ctx context.Context, query *MainIotQuery, nodes []*User, init func(*User), assign func(*User, *MainIot)) error {
	...
	query.withFKs = true
	query.Where(predicate.MainIot(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.MainiotsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_mainiots
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_mainiots" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
